...
public class MoltenCrimsiteBlockEntity extends BlockEntity {
    ...
    private static void spreadBlock(Block requiredLiquid, BlockPos pos, WorldGenLevel level) {
        ...
        if (isTouchingCrimsite && !CommonConfig.useSource.get() && level.getBlockState(pos).is(requiredLiquid) || isTouchingCrimsite && CommonConfig.useSource.get() && level.getBlockState(pos).is(requiredLiquid) && level.getBlockState(pos).getFluidState().isSource() || isTouchingCrimsite && level.getBlockState(pos).is(MoltenGeodesFeatures.Crimsite)) {
    ...
    private static List<BlockPos> getLavaTouching(Block requiredLiquid, BlockPos pos, WorldGenLevel level) {
        ...
        for (int i = 0; i < contacts.length; ++i) { //Check each non-diagonal neighbor to see if it is crimsite
            if (!CommonConfig.useSource.get() && level.getBlockState(contacts[i]).is(requiredLiquid) || CommonConfig.useSource.get() && level.getBlockState(contacts[i]).is(requiredLiquid) && level.getBlockState(contacts[i]).getFluidState().isSource() || level.getBlockState(contacts[i]).is(MoltenGeodesFeatures.Crimsite)) {
